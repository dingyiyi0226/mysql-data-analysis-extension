 ...
 *
 * Description:
 *   Fit the data by Y = a * exp(-lambda * X) + b
 *
 * Usage:
 *   FIT_EXP(`X`, `Y`, data_length)
 *
 * Return:
 *   "a lambda b": char * (STRING)
 *
 */
...
extern "C" {
    bool fit_exp_init(UDF_INIT *initid,UDF_ARGS *args, char *message);
    void fit_exp_deinit(UDF_INIT *initid);
    void fit_exp_clear(UDF_INIT *initid, char *is_null, char *error);
    void fit_exp_add(UDF_INIT *initid, UDF_ARGS *args, char *is_null, char *error);
    char *fit_exp(UDF_INIT *initid, UDF_ARGS*args, char *result, unsigned long *length, char *is_null, char *error);
}
...
int exp_f(const gsl_vector *x, void *data, gsl_vector *f){
    size_t n = ((fit_exp_data *)data)->length;
    gsl_vector *data_x = ((fit_exp_data *)data)->x;
    gsl_vector *data_y = ((fit_exp_data *)data)->y;
    ...
    for(int i=0; i<n; i++){
        Yi = A * exp (-lambda * gsl_vector_get(data_x, i)) + b;
...
bool fit_exp_init(UDF_INIT *initid,UDF_ARGS *args, char *message){
    if(args->arg_count != 3){
        strcpy(message, "wrong number of arguments: FIT_EXP() requires three arguments");
        return 1;
    }
    ...
    fit_exp_data *data = new fit_exp_data;

    data->length = *(int *)args->args[2];
    data->var_num = 3;  // A, lambda, B
    data->nth_row = 0;
...
void fit_exp_deinit(UDF_INIT *initid){
    fit_exp_data *data = (fit_exp_data *)initid->ptr;
...
char *fit_exp(UDF_INIT *initid, UDF_ARGS *args, char *result, unsigned long *length, char *is_null, char *error){
    ...
    *length = sprintf(result, "%f %f %f",
        gsl_vector_get(data->w->x, 0), gsl_vector_get(data->w->x, 1), gsl_vector_get(data->w->x, 2));
